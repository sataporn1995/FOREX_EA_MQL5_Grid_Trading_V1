...
input bool     InputEnableZones  = true;    // Enable Zone: true = Enable & false = Disable
...
input bool     InputEnableTP  = false;    // Enable TP: true = Enable & false = Disable
...
bool MaybeOpenNext(Direction dir)
{
   ...
   if(dir==DIR_BUY)
   {
      ...
      if(tk.ask <= target && !HasOrderNear(target))
      {
         ...
         if(!Trade.Buy(CalLotByZones(), Symb, tk.ask, 0, tpPrice, cmt))
   ...
   else // DIR_SELL
   {
      ...
      if(tk.bid >= target && !HasOrderNear(target))
      {
         ...
         if(!Trade.Sell(CalLotByZones(), Symb, tk.bid, 0, tpPrice, cmt))
