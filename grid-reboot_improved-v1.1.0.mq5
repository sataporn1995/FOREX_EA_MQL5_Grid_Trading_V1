...
enum GridTypeEnum { 
   GRID_AVG_CLOSE, // Close All
   GRID_TP // Grid TP
};
enum TrendEnum { UPTREND=1, DOWNTREND=-1, SIDEWAY=0 };
enum SumNetEnum { 
   SUM_NET_POINTS,  // Net Profit (Points)
   SUM_NET_AMOUNT   // Net Profit (Money)
};

input group "=== GRID SETTINGS ==="; 
input DirectionEnum InpDirection = DIR_BOTH; // Trade Direction
input GridTypeEnum InpGridType = GRID_AVG_CLOSE; // Grid Type
input int InpGridStepPoints = 10000; // Grid Step (Points)
input int InpMaxOrders = 0; // Max Order (0 = Unlimited)
// ความปลอดภัย: ป้องกันเปิดซ้ำระดับเดิม (เช็คช่วงกันชน 15% ของกริด)
input double InpNoDupLevelRatio  = 0.0; // GridStep is the buffer period // 0 = Do not block the price range, enter the order.

input group "=== + Grid Step with ATR ==="
input bool   InpEnableATR = true;           // Use ATR for SL
...
input group "=== PROFIT ==="; 
input SumNetEnum InpSumNetType = SUM_NET_AMOUNT; // Profit Type
input int InpProfitTargetPts = 3000; // Profit or TP (Points)
input double InpProfitTargetAmount = 10.0; // Profit Money
...
input bool InpEnableNewBar = false; // Enable Open 1st Order on New Bar
input ENUM_TIMEFRAMES  InpNewBarTF = PERIOD_M1; // TF for New Bar

input group "=== ZONE FILTER ===";
input bool InpEnablePriceZone = false; // Enable Zone for Trade
input double InpUpperPrice = 0.0; // Upper Price (0 = No limit)
input double InpLowerPrice = 0.0; // Lower Price (0 = No limit)

input group "=== RSI FILTER ===";
input bool InpEnableRsiFilter = true; // Enable RSI Filter
input ENUM_TIMEFRAMES InpRsiTF = PERIOD_M5; // TF for RSI
...
input bool InpEnableStochFilter = false; // Enable STO Filter
input ENUM_TIMEFRAMES InpStochTF = PERIOD_M1; // TF for STO
...
enum ENUM_TRADE_TREND {
   IGNORE_TREND, // Ignore Trend
   FOLLOW_TREND, // Follow Trend
   COUNTER_TREND // Counter Trend
};
input ENUM_TRADE_TREND InpTradeTrend = FOLLOW_TREND; // Trade Trend
input ENUM_TIMEFRAMES  InpTrendTF = PERIOD_H1; // TF for Trend
input int InpEmaFast = 50; // EMA Fast
input int InpEmaSlow = 150; // EMA Slow

input group "=== OTHER ===";
input long InpMagic = 2025110701; // Magic number
input int InpSlippage = 20; // Slippage (points)
input bool InpCommentPriceLvl = true; // Write comment in price level
...
bool MaybeOpenNext(ENUM_POSITION_TYPE posType)
{
   ...
   double atr = CalATRDistance();

   if(posType == POSITION_TYPE_BUY)
   {
      double target = lastOpen - InpGridStepPoints*_Point - atr;
      if(tk.ask <= target && !HasOrderNear(target, posType))
      {
         double tpPrice = 0;
         if (InpGridType == GRID_TP)
            tpPrice = CalTagetPrice(tk.ask, InpProfitTargetPts, true) + atr;
   ...
   else if(posType == POSITION_TYPE_SELL)
   {
      ...
      if(tk.bid >= target && !HasOrderNear(target, posType))
      {
         double tpPrice = 0;
         if (InpGridType == GRID_TP)
            tpPrice = CalTagetPrice(tk.bid, InpProfitTargetPts, false) - atr;
...
{
   if(InpEnableATR && CopyBuffer(atrHandle, 0, 0, 2, atrValue) < 2) return false;
   
   return true;
}

double CalATRDistance() {
   double value = 0;
   if (InpEnableATR && atrValue[0] > 0) value = atrValue[0] * InpATRMultiplier;
   //long points = (long)MathRound(value / _Point);
   return value;
}
...
bool ShouldOpenOrder(DirectionEnum dir)
{
   ...
   if (dir == DIR_BUY || dir == DIR_BOTH)
   {
      ...
      if (InpTradeTrend == FOLLOW_TREND && currentTrend != UPTREND && dir == DIR_BUY) return false;
      if (InpTradeTrend == COUNTER_TREND && currentTrend != DOWNTREND && dir == DIR_BUY) return false;
   }
   ...
   if (dir == DIR_SELL || dir == DIR_BOTH)
   {
      ...
      if (InpTradeTrend == FOLLOW_TREND && currentTrend != DOWNTREND && dir == DIR_SELL) return false;
      if (InpTradeTrend == COUNTER_TREND && currentTrend != UPTREND && dir == DIR_SELL) return false;
   }
...
int OnInit()
{
   ...
   atrHandle = iATR(_Symbol, InpATRTF, InpARTPeriod);
