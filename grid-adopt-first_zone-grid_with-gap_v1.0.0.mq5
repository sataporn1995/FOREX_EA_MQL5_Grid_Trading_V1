...
struct ZoneConfig
{
  ...
  int zoneGaps[];        // ระยะห่างระหว่างโซน (จุด) - มี n-1 ค่าสำหรับ n โซน
...
bool ParseZoneConfig()
{
  ...
  // Parse ระยะห่างระหว่างโซน (ต้องมี n-1 ค่าสำหรับ n โซน)
  string gapArr[];
  int gapCount = 0;
  if(StringLen(ZoneGapInput) > 0)
  {
    gapCount = StringSplit(ZoneGapInput, ',', gapArr);
    if(gapCount != ZoneCount - 1)
    {
      Print("ERROR: จำนวน Zone Gap ต้องเป็น ", ZoneCount-1, " (น้อยกว่าจำนวนโซน 1)");
      return false;
    }
  }
  ...
  for(int i=0; i<gapCount; i++)
  {
    ...
    {
      Print("ERROR: ค่า Zone Gap ไม่ถูกต้องระหว่างโซน ", i+1, " และ ", i+2);
      return false;
    }
  }
...
bool ShouldOpenAtZone(const PosInfo &first, int currentOrderCount, double currentPrice, int &outZone, double &outTargetPrice)
{
  ...
  for(int z=0; z<g_ZoneConfig.totalZones; z++)
  {
    ...
    if(currentOrderCount < totalNeeded)
    {
      ...
      // คำนวณราคาเป้าหมาย โดยรวม Zone Gap
      double distancePoints = 0;
      
      // รวมระยะจากโซนก่อนหน้า + Gap ระหว่างโซน
      for(int prevZ=0; prevZ<z; prevZ++)
      {
        // ระยะของออเดอร์ทั้งหมดในโซนก่อนหน้า
        distancePoints += g_ZoneConfig.zoneOrders[prevZ] * g_ZoneConfig.zoneSpacing[prevZ];
        
        // เพิ่ม Gap ระหว่างโซน (ถ้ามี)
        if(prevZ < ArraySize(g_ZoneConfig.zoneGaps))
        {
          distancePoints += g_ZoneConfig.zoneGaps[prevZ];
        }
      }
      
      // เพิ่มระยะของออเดอร์ในโซนปัจจุบัน
